...
class TicTacToe:
    ...
    def winner(self, square, letter):
        # winner if 3 in a row anywhere.. we have to check all the possibilities
...
def play(game, x_player, o_player, print_game=True):
    ...
    while game.empty_squares():
        ...
        if game.make_move(square, letter):
            ...
            # after we made or move, we need to alternate letters
            letter = 'O' if letter == 'X' else 'X'  # switches player

        # tiny break, to make thins easier to read
        if print_game:
            time.sleep(0.8)
